...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
		...
		//获取应用配置
//		$config = config("status.THINK_SDK_{$this->Type}");
...
//		}
        //获取微信应用配置
		if(empty(config("status.THINK_SDK_WEIXIN.APP_KEY")) || empty(config("status.THINK_SDK_WEIXIN.APP_SECRET"))){
			throw new Exception('请配置您申请的APP_KEY和APP_SECRET');
		} else {
			$this->AppKey    = config("status.THINK_SDK_WEIXIN.APP_KEY");
			$this->AppSecret = config("status.THINK_SDK_WEIXIN.APP_SECRET");
			$this->Token     = $token; //设置获取到的TOKEN
		}
	}
    ...
    public static function getInstance($type, $token = null) {
    	...
    	return new sdk\WeixinSDK($token);
    	//require_once dirname(__FILE__) ."\sdk\\$name.php";
    	//var_dump(dirname(__FILE__) ."\sdk\\$name.php");die();
